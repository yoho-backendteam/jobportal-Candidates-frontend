...
const Active = () => {
  ...
  const [ids, setIds] = useState(alljobs[0]?._id);

  const timeline: any = useSelector(selectTimeline) || [];

  const handleTimeline = (id: any) => {
    setopen(!open);
    setIds(id);
  };
  ...
  return (
    <div className="space-y-6 p-5">
      ...
      <div>
        {alljobs?.map((value: any, index: number) => {
          return (
            <div
              key={index}
            ...
            >
              ...
              <button
                ...
                onClick={() => handleTimeline(value?._id)}
              >
      ...
      {open && (
        <div className="space-y-5">
          <h1>Your Applied Job TimeLine for {timeline?.data?.jobTitle}</h1>
